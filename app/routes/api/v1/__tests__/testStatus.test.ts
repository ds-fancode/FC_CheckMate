import { loader } from "~/routes/api/v1/testStatus"; // Adjust the path as necessary
...
describe("Get Test Status - Loader Function", () => {
  ...
  it("should successfully fetch test status for valid query parameters", async () => {
    ...
    (TestsController.getTestStatus as jest.Mock).mockResolvedValue(mockTestStatusData);
    ...
    expect(SearchParams.getTestStatus).toHaveBeenCalledWith({ params: {}, request });
  ...
  it("should return an error for invalid query parameters", async () => {
    const request = new Request("http://localhost?projectId=invalid&testId=456", {
      method: "GET",
    });
