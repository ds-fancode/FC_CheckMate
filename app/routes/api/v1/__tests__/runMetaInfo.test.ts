import { loader } from "~/routes/api/v1/runMetaInfo"; // Adjust the path as necessary
...
describe("Get Run State Detail - Loader Function", () => {
  ...
  it("should successfully fetch test run state details for a valid runId and groupBy", async () => {
    const request = new Request("http://localhost?runId=123&projectId=456&groupBy=squads", {
      method: "GET",
    });
    ...
    (TestRunsController.runsMetaInfo as jest.Mock).mockResolvedValue(mockTestRunMetaData);
  ...
  it("should return an error if groupBy is invalid", async () => {
    const request = new Request("http://localhost?runId=123&projectId=456&groupBy=invalid", {
      method: "GET",
    });
  ...
  it("should return an error if runId is invalid", async () => {
    const request = new Request("http://localhost?runId=invalid&projectId=456", {
      method: "GET",
    });
  ...
  it("should return an error if runId is missing", async () => {
    const request = new Request("http://localhost?projectId=456", { method: "GET" });
  ...
  it("should handle unexpected errors", async () => {
    const request = new Request("http://localhost?runId=123&projectId=456", { method: "GET" });
