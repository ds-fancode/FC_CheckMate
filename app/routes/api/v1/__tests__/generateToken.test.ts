import { action } from "~/routes/api/v1/generateToken"; // Adjust the path as necessary
...
describe("Generate Token - Action Function", () => {
  ...
  it("should return an error if userId in the token does not match the authenticated user", async () => {
    const requestData = { userId: 456 }; // Different userId
    const request = new Request("http://localhost", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(requestData),
  ...
  it("should handle validation errors", async () => {
    const invalidRequestData = { userId: -1 }; // Invalid userId
    const request = new Request("http://localhost", {
      method: "POST",
      headers: { "content-type": "application/json" },
