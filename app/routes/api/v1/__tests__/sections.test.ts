import { loader } from "~/routes/api/v1/sections"; // Adjust the path as necessary
...
describe("Get Sections - Loader Function", () => {
  ...
  it("should successfully fetch sections for a valid projectId", async () => {
    const request = new Request("http://localhost?projectId=123", { method: "GET" });
    ...
    (SectionsController.getAllSections as jest.Mock).mockResolvedValue(mockSectionsData);
    ...
    expect(SectionsController.getAllSections).toHaveBeenCalledWith({ projectId: 123 });
  ...
  it("should throw an error for invalid projectId", async () => {
    ...
    (errorResponseHandler as jest.Mock).mockImplementation((error) =>
      new Response(JSON.stringify({ error: error.message, cause: error.cause }), { status: 400 })
    );
