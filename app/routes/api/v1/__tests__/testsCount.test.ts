import { loader } from "~/routes/api/v1/testsCount"; // Adjust the path as necessary
...
describe("Get Tests Count - Loader Function", () => {
  ...
  it("should successfully fetch test count for valid search parameters", async () => {
    const request = new Request("http://localhost?runId=123&page=1&pageSize=10", {
      method: "GET",
    });
    ...
    (TestsController.getTestsCount as jest.Mock).mockResolvedValue(mockTestsCount);
    ...
    expect(SearchParams.getTestCount).toHaveBeenCalledWith({ params: {}, request });
  ...
  it("should log and return an error for invalid parameters", async () => {
    ...
    (SearchParams.getTestCount as jest.Mock).mockImplementation(() => {
      throw new Error("Invalid search parameters", { cause: ErrorCause.INVALID_PARAMS });
    });
