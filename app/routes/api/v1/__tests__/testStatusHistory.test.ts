import { loader } from "~/routes/api/v1/testStatusHistory"; // Adjust the path as necessary
...
describe("Get Test Status History - Loader Function", () => {
  ...
  it("should successfully fetch test status history for a valid testId", async () => {
    ...
    (TestRunsController.testStatusHistory as jest.Mock).mockResolvedValue(mockTestStatusData);
    ...
    expect(TestRunsController.testStatusHistory).toHaveBeenCalledWith({ testId: 123 });
  ...
  it("should return an error for an invalid testId", async () => {
    const request = new Request("http://localhost?testId=invalid", { method: "GET" });
