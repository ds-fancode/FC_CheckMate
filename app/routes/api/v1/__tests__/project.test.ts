import { loader } from "~/routes/api/v1/projects"; // Adjust the path as necessary
...
describe("Get Projects - Loader Function", () => {
  ...
  it("should successfully fetch projects for valid parameters", async () => {
    ...
    (ProjectsController.getAllProjects as jest.Mock).mockResolvedValue(mockProjectsData);
  ...
  it("should throw an error for invalid page or pageSize", async () => {
    const request = new Request("http://localhost?orgId=123&page=0&pageSize=-1", {
      method: "GET",
    });
  ...
  it("should handle unexpected errors", async () => {
    const request = new Request("http://localhost?orgId=123&page=1&pageSize=10", {
      method: "GET",
    });
