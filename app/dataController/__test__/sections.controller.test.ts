import SectionsController from "@controllers/sections.controller"; // Adjust the path as necessary
...
describe("SectionsController", () => {
  ...
  describe("getSectionIdByNameAndHierarcy", () => {
    it("should call SectionsDao.getSectionIdByNameAndHierarcy with the correct parameters", async () => {
      ...
      (SectionsDao.getSectionIdByNameAndHierarcy as jest.Mock).mockResolvedValue(mockResponse);

      const result = await SectionsController.getSectionIdByNameAndHierarcy(mockParams);

      expect(SectionsDao.getSectionIdByNameAndHierarcy).toHaveBeenCalledWith(mockParams);
      expect(result).toEqual(mockResponse);
    });
  });
  ...
  describe("checkAndCreateSection", () => {
    ...
    it("should create a new section if not found", async () => {
      ...
      (SectionsDao.getSectionIdByNameAndHierarcy as jest.Mock).mockResolvedValue([]);
      (SectionsDao.addSection as jest.Mock).mockResolvedValue(mockResponse);
