import { UserRoleStore } from "~/services/rbac/userRoleStore"; // Adjust the path as necessary
...
describe("UserRoleStore", () => {
  ...
  describe("addUserRole", () => {
    ...
    it("should not duplicate user roles", () => {
      ...
      expect((userRoleStore as any).userRoles.size).toBe(1); // Only one entry
    });
    ...
    it("should allow multiple roles for the same user", () => {
      ...
      expect((userRoleStore as any).userRoles.size).toBe(2); // Two unique entries
    });
  });
